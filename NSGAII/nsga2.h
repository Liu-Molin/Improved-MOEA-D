...
struct population{
    ...
    std::vector<std::vector<struct solution> >d_front;
...
bool util_dominate(const struct solution &x, const struct solution&y){
    if((x.objectives[0]>=y.objectives[0])&&x.objectives[1]<=y.objectives[1]){
        return true;
    }
    return false;
}
...
void util_fundDistribute( std::vector<struct asset>&p, double&fund, int n, int mode){
    ...
    if(n<=0||mode == 1){
        rn:
        util_fundDistribute(p, fund, int(randG()*p.size()),1);
    }
    else {
...
void util_ffunction(solution &xi, const std::vector<asset> &assetArray){
    double income = 0.0;
    double risk = 0.0;
    for(int i = 0; i<xi.gene.size(); i++){
        if(xi.gene[i]!=0){
...
void init_population(struct population &x,
    ...
    for(int i = 0; i<N_local; i++){
        ...
        //double risk_buffer = 0;

        util_ffunction(xi_buffer, asset);
...
void gene2txt(std::string outPath, population&x){
    std::ofstream outFile;
    outFile.open(outPath);
    for(int i = 0; i<x.xi[0].gene.size(); i++){
        outFile<<i<<"\t";
    }
    outFile<<"Risk\tIncome\n";
    for(auto item:x.xi){
        std::string tempLine = "";
        for(int i = 0; i<item.gene.size(); i++){
            tempLine+=std::to_string(item.gene[i])+"\t";
        }
        outFile<<tempLine<<item.objectives[1]<<"\t"<<item.objectives[0]<<"\n";
    }
    outFile.close();
}
